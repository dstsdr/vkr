    ...
    public partial class prodazhi__ : Form
    {
        ...
        private void bezrecepttovar ()
        {
            ...
            cmd4.CommandText = "SELECT CONCAT ([Характеристики лекарств].[Код характеристики] , ' ',Лекарства.Наименование, ' ', " +
                ...
                "[Условие отпуска].[Код условия] = 2 " +
                "GROUP BY [Характеристики лекарств].Дозировка, Производитель.Наименование , " +
        ...
        private void recepttovar()
        {
            ...
            cmd4.CommandText = "SELECT CONCAT ([Характеристики лекарств].[Код характеристики] , ' ',Лекарства.Наименование, " +
                ...
                "Производитель.Наименование) as ДА " +
                "from[Характеристики лекарств] " +
                "inner join(Лекарства " +
                "inner join[Условие отпуска] ON " + "[Условие отпуска].[Код условия]= Лекарства.[Код условия]) " +
                "ON Лекарства.[Код лекарства] =" + "[Характеристики лекарств].[Код лекарства] " +
                ...
                "inner join Производитель ON Производитель.[Код производителя] =" + "[Характеристики лекарств].[Код производителя] " +
                "inner join[Серийный номер] ON[Характеристики лекарств].[Код характеристики] " + "=[Серийный номер].[Код характеристики] " +
                "WHERE[Серийный номер].[Код рецептурной продажи] is null and " + "[Серийный номер].[Код безрецептурной продажи] is null and [Условие отпуска].[Код условия] = 1 " +
                "GROUP BY" + "[Характеристики лекарств].Дозировка, Производитель.Наименование , [Характеристики лекарств].Наценка, " +
                "Лекарства.Наименование, [Форма выпуска].Форма, [Характеристики лекарств].[Номер договора], [Единицы измерения].Обозначение, " +
                "[Характеристики лекарств].[Код характеристики] " +
                "ORDER BY ДА ASC";
            cmd4.ExecuteNonQuery();
            DataTable dt4 = new DataTable();
            SqlDataAdapter da4 = new SqlDataAdapter(cmd4);
        ...
        private void skidki(string id)
        {
            ...
            cmd3.CommandText = "SELECT CONCAT (Лекарства.Наименование, ' ' ,[Форма выпуска].Форма, ' ',Скидки.Размер, '% от ', Скидки.[Минимальное количество], ' шт.') AS СКИДКИ " +
                ...
                "ON[Характеристики лекарств].[Код характеристики] = " + "Скидки.[Код характеристики] " +
                "WHERE [Характеристики лекарств].[Код характеристики] =" + id;
        ...
        private void seriya(string id)
        {
            ...
            SqlCommand cmd4 = Connection.CreateCommand();
            cmd4.CommandType = CommandType.Text;
            cmd4.CommandText = "SELECT [Серийный номер].[Серийный номер] AS ДА " +
                "from[Серийный номер] " +
                "inner join[Характеристики лекарств] " +
            ...
            Connection.Open();
             for (int i = 0; i < checkedListBox1.Items.Count; i++)
             {
                 if (checkedListBox1.GetItemChecked(i))
                 {
                     SqlCommand cmd3 = Connection.CreateCommand();
                     cmd3.CommandType = CommandType.Text;
                     cmd3.CommandText = "select [Характеристики лекарств].[Код характеристики] AS НОМЕР " +
                        "from[Характеристики лекарств] " +
                        "inner join[Серийный номер] " + "ON[Характеристики лекарств].[Код характеристики] =[Серийный номер].[Код характеристики] " +
                        "where[Серийный номер].[Серийный номер] = " + (checkedListBox1.Items[i]).ToString();
                     cmd3.ExecuteNonQuery();
                     DataTable dt3 = new DataTable();
                     SqlDataAdapter da3 = new SqlDataAdapter(cmd3);
                     da3.Fill(dt3);
        ...
        private void rashet ()
        {
            ...
            for (int i= 0; i<dataGridView1.RowCount-1; i++)
            {
                SqlCommand cmd4 = Connection.CreateCommand();
                cmd4.CommandType = CommandType.Text;
                cmd4.CommandText = "SELECT [Характеристики лекарств].Цена AS ЦЕНА " +
                    "from [Характеристики лекарств] " +
                    "where [Характеристики лекарств].[Код характеристики] =" + dataGridView1[0, i].Value.ToString();
                cmd4.ExecuteNonQuery();
                DataTable dt4 = new DataTable();
                SqlDataAdapter da4 = new SqlDataAdapter(cmd4);
                da4.Fill(dt4);
