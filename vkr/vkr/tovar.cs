    ...
    public partial class tovar : Form
    {
        ...
        private void dataset()
        {
            ...
            SqlDataAdapter adapter = new SqlDataAdapter("SELECT [Серийный номер].[Серийный номер], [Серийный номер].[Срок годности], [Характеристики лекарств].Дозировка," +
                ...
                "AS[Фарм группа], [Форма выпуска].Форма  AS[Форма выпуска], Производитель.Наименование AS[Производитель], [Условие отпуска].Условие " +
                ...
                "WHERE [Серийный номер].[Код рецептурной продажи] " + "is null and [Серийный номер].[Код безрецептурной продажи] is null  and [Серийный номер].[Срок годности]>GETDATE()", Connection);
        ...
        private void checkBox6_CheckedChanged(object sender, EventArgs e)
        {
            if (checkBox6.Checked == true)
            {
                ...
                SqlDataAdapter adapter = new SqlDataAdapter("SELECT Поставщик.* " +
                    "from[Характеристики лекарств] " +
                    "inner join[Серийный номер] ON[Серийный номер].[Код характеристики] =[Характеристики лекарств].[Код характеристики] " +
        ...
        private void checkBox5_CheckedChanged(object sender, EventArgs e)
        {
            if (checkBox5.CheckState == CheckState.Checked)
            {
                ...
                SqlDataAdapter adapter = new SqlDataAdapter("SELECT [Серийный номер].[Серийный номер], [Серийный номер].[Срок годности], [Характеристики лекарств].Дозировка, " +
                    ...
                    "inner join[Условие отпуска] ON Лекарства.[Код условия] =[Условие отпуска].[Код условия] " +
                    "inner join[Фарм группа] ON Лекарства.[Код группы] =" +                    "[Фарм группа].[Код группы] " +
                    "inner join Наценка ON Наценка.[Код наценки] = Лекарства.[Код наценки]) ON Лекарства.[Код лекарства] =[Характеристики лекарств].[Код лекарства]" +
                    " inner join[Форма выпуска] ON [Форма выпуска].[Код формы] =[Характеристики лекарств].[Код формы] " +
                    "inner join Производитель ON Производитель.[Код производителя] " +                     "=[Характеристики лекарств].[Код производителя]" +
                    " inner join[Серийный номер] ON[Характеристики лекарств].[Код характеристики] =[Серийный номер].[Код характеристики] " +
