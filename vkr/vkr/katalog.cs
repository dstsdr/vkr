    ...
    public partial class katalog : Form
    {
        ...
        private void dataset()
        {
            ...
            SqlDataAdapter adapter = new SqlDataAdapter("SELECT COUNT([Серийный номер].[Код характеристики]) AS [Кол-во], Лекарства.Наименование, [Фарм группа].Название AS [Фарм группа], " +
                ...
                " [Условие отпуска].Условие, Лекарства.[Похожее лекарство], [Характеристики лекарств].Цена  from[Характеристики лекарств] inner join(Лекарства inner join[Условие отпуска] ON Лекарства.[Код условия] =[Условие отпуска].[Код условия]" +
                " inner join[Фарм группа] ON Лекарства.[Код группы] =[Фарм группа].[Код группы]) ON Лекарства.[Код лекарства] =[Характеристики лекарств].[Код лекарства] inner join" +
                "([Форма выпуска] inner join[Единицы измерения] ON[Единицы измерения].[Код ед.изм] =[Форма выпуска].[Код ед.изм])ON[Форма выпуска].[Код формы] =" +
                "[Характеристики лекарств].[Код формы] inner join Производитель ON Производитель.[Код производителя] =[Характеристики лекарств].[Код производителя] inner join" +
                "[Серийный номер] ON[Характеристики лекарств].[Код характеристики] =[Серийный номер].[Код характеристики] where[Серийный номер].[Срок годности] > GETDATE() " +
                "and[Код безрецептурной продажи] is null and [Код рецептурной продажи] is null GROUP BY[Характеристики лекарств].[Код характеристики], Лекарства.Наименование, " +
                "[Фарм группа].Название, [Форма выпуска].Форма, Производитель.Наименование, [Характеристики лекарств].Дозировка, Лекарства.[Похожее лекарство], " +
                "[Единицы измерения].Обозначение, [Условие отпуска].Условие, [Характеристики лекарств].Цена", Connection);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter adapter = new SqlDataAdapter("SELECT [Характеристики лекарств].[Код характеристики] as '№',Лекарства.Наименование, " +
                ...
                "Производитель.Наименование  AS[Производитель], CONCAT([Характеристики лекарств].Дозировка, ' ', [Единицы измерения].Обозначение), " +
                "Договор.Количество, [Характеристики лекарств].Цена from[Характеристики лекарств] inner join(Лекарства inner join[Условие отпуска] ON " +
                "Лекарства.[Код условия] =[Условие отпуска].[Код условия] inner join[Фарм группа] ON Лекарства.[Код группы] =[Фарм группа].[Код группы]) " +
                "ON Лекарства.[Код лекарства] =[Характеристики лекарств].[Код лекарства] inner join([Форма выпуска] inner join [Единицы измерения] " +
                "ON[Единицы измерения].[Код ед.изм]=[Форма выпуска].[Код ед.изм])ON[Форма выпуска].[Код формы] =[Характеристики лекарств].[Код формы] " +
                "inner join Производитель ON Производитель.[Код производителя] =[Характеристики лекарств].[Код производителя] inner join Договор ON " +
                "Договор.[Номер договора] =[Характеристики лекарств].[Номер договора] WHERE Лекарства.[Похожее лекарство]='" + dataGridView1.CurrentRow.Cells[8].Value + "'", Connection);
            DataSet ds = new DataSet();
            adapter.Fill(ds, "info");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            cmd8.CommandText = "SELECT [Характеристики лекарств].[Код характеристики] as Код from[Характеристики лекарств] inner join[Форма выпуска] ON[Характеристики лекарств].[Код формы]" +
                " =[Форма выпуска].[Код формы] inner join(Лекарства inner join[Условие отпуска] ON [Условие отпуска].[Код условия]= Лекарства.[Код условия]) " +
                "ON Лекарства.[Код лекарства]=[Характеристики лекарств].[Код лекарства] inner join Производитель ON Производитель.[Код производителя] =" +
                "[Характеристики лекарств].[Код производителя] where[Лекарства].Наименование = '"+ dataGridView1.CurrentRow.Cells[1].Value+"' and [Форма выпуска].Форма = '"+dataGridView1.CurrentRow.Cells[3].Value +"' " +
                ...
                "and [Условие отпуска].Условие = '"+ dataGridView1.CurrentRow.Cells[7].Value+"' " + "and [Характеристики лекарств].Цена ="+ dataGridView1.CurrentRow.Cells[9].Value;
        ...
        private void checkBox1_CheckedChanged(object sender, EventArgs e)
        {
            if (checkBox1.Checked == true)
            {
                ...
                SqlDataAdapter adapter = new SqlDataAdapter("SELECT [Серийный номер].[Серийный номер], [Серийный номер].[Срок годности], [Характеристики лекарств].Наценка " +
                    ...
                    " ' шт.') as Скидки from[Характеристики лекарств] inner join(Лекарства inner join[Условие отпуска] ON Лекарства.[Код условия] =" +
                    "[Условие отпуска].[Код условия] inner join[Фарм группа] ON Лекарства.[Код группы] =[Фарм группа].[Код группы] inner join Наценка ON " +
                    ...
                    "[Форма выпуска].[Код формы] =[Характеристики лекарств].[Код формы] inner join Производитель ON Производитель.[Код производителя] =" +
                    "[Характеристики лекарств].[Код производителя] inner join[Серийный номер] ON[Характеристики лекарств].[Код характеристики] =" +
                    "[Серийный номер].[Код характеристики] inner join Скидки ON Скидки.[Код характеристики] =[Характеристики лекарств].[Код характеристики] " +
        ...
        private void checkBox5_CheckedChanged(object sender, EventArgs e)
        {
            if (checkBox5.CheckState == CheckState.Checked)
            {
                SqlDataAdapter adapter = new SqlDataAdapter("SELECT COUNT([Серийный номер].[Код характеристики]) AS [Кол-во], Лекарства.Наименование, [Фарм группа].Название AS [Фарм группа]," +
                    ...
                    "'Рецептурное' GROUP BY[Характеристики лекарств].[Код характеристики], Лекарства.Наименование, [Фарм группа].Название, [Форма выпуска].Форма, " +
        ...
        private void checkBox7_CheckedChanged(object sender, EventArgs e)
        {
            if (checkBox7.CheckState == CheckState.Checked)
            {
                ...
                SqlDataAdapter adapter = new SqlDataAdapter("SELECT COUNT([Серийный номер].[Код характеристики]) AS [Кол-во], Лекарства.Наименование, [Фарм группа].Название AS [Фарм группа]," +
                    "[Форма выпуска].Форма AS[Форма выпуска], [Характеристики лекарств].Дозировка,[Единицы измерения].Обозначение, Производитель.Наименование AS[Производитель], " +
                    "[Условие отпуска].Условие, Лекарства.[Похожее лекарство], [Характеристики лекарств].Цена, [Необходимый минимум].[Код лекарства] from[Характеристики лекарств] inner join" +
                    "(Лекарства inner join[Условие отпуска] ON Лекарства.[Код условия] =[Условие отпуска].[Код условия] inner join[Фарм группа] ON Лекарства.[Код группы] =" +
                    "[Фарм группа].[Код группы] inner join[Необходимый минимум] on[Необходимый минимум].[Код лекарства] = Лекарства.[Код лекарства]) ON Лекарства.[Код лекарства] =" +
                    "[Характеристики лекарств].[Код лекарства] inner join ([Форма выпуска] inner join[Единицы измерения] ON[Единицы измерения].[Код ед.изм] =[Форма выпуска].[Код ед.изм])" +
                    "ON[Форма выпуска].[Код формы] = [Характеристики лекарств].[Код формы] inner join Производитель ON Производитель.[Код производителя] =" +
                    "[Характеристики лекарств].[Код производителя] inner join [Серийный номер] ON[Характеристики лекарств].[Код характеристики] =[Серийный номер].[Код характеристики] " +
                    ...
                    " is not null GROUP BY[Характеристики лекарств].[Код характеристики], Лекарства.Наименование, [Фарм группа].Название, [Форма выпуска].Форма, Производитель.Наименование," +
